...
import mandomc.mmccore.handlers.UtilityMethods;
...
public class TieFighter implements Listener {
    ...
    public void createShip(Player player){
        ...
        if(isMobSpawningEnabled(player.getLocation(), player) && !player.getWorld().getName().equals("JabbasPalace")){
            ...
            Vehicles.entitiesInShip.add(tieFighter.getSeat1());
            Vehicles.armorStandsInShip.add(tieFighter.getModel());
    ...
    public void rideShip(Player player, Vehicle tieFighter){
        ...
        Vehicles.playersInShip.add(tieFighter.getPilot());

        Entity seat1 = tieFighter.getSeat1();
        LivingEntity seat1Living = (Phantom) seat1;
        seat1Living.addPassenger(player);

        player.sendMessage(MMCCore.prefix + ChatColor.translateAlternateColorCodes('&', "&7You mounted your &8Tie-Fighter&7!"));
    }
    ...
    public static void removeShip(Player player, Vehicle tieFighter){
        ...
        Vehicles.armorStandsInShip.remove(model);
        ...
        player.sendMessage(MMCCore.prefix + ChatColor.translateAlternateColorCodes('&', "&7You dismounted your &8Tie-Fighter&7!"));
    }
                ...
                if (loc.getBlock().getType().isSolid() || t == 127) {
                    ...
                    if (isMobSpawningEnabled(loc, player)) {
                        for (Entity entity : loc.getWorld().getNearbyEntities(loc, 8.0, 8.0, 8.0)) {
                            Vector vector = UtilityMethods.genVec(player.getLocation(), entity.getLocation());
                            vector.setY(1.2);
